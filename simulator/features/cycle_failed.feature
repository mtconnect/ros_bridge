...
Feature: Machine Tool cycle failure
  ...
  Background: Cnc begins a cycle
    ...
    And robot MaterialHandlerInterface MaterialLoad becomes Active
    And Chuck is closed
    And Door is closed
    And robot MaterialHandlerInterface MaterialLoad becomes Complete
    And robot MaterialHandlerInterface MaterialLoad becomes Ready
  ...
  Scenario: Cycle starts and then alarms out
    ...
    And cnc Controller Execution is Active

    When cnc faults Controller MOTION_PROGRAM with "Program not loaded"
    And cnc Controller Execution is Stopped
  ...
  Scenario: Cycle starts and then alarms out and the alarm clears
    ...
    When cnc clears Controller MOTION_PROGRAM
    When cnc is reset
