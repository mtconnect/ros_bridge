...
//
		...
		namespace submachines
		{
			...
			//-------------------  Operational Sub-machine front end ---------------------------------------//
			...
			struct MaterialLoad_ : public MaterialHandling_
			{
				...
				bool verify_completion_and_proceed(int task_id)
				{
					...
					if(task_list_[current_task_index_++] == task_id)
					{
						...
						{
							std::cout<<indenting_str_<<name_<<" completed task "<<current_task_index_ - 1<<", with id: "<<task_id<<std::endl;
							dispatch_task(task_list_[current_task_index_]);
							std::cout<<indenting_str_<<name_<<" dispatching task "<<current_task_index_ <<", with id: "<<task_list_[current_task_index_]<<std::endl;
						}
						else
						{
							// resetting variables
				...
				virtual void handle_event(robot_move_completed const &evnt)
				{
					...
					if(!evnt.success_)
					{
						...
						ref.process_event(events::material_load_failed());
						//ref.process_event(events::robot_fault_detected());
					}
					else
					{
						...
						{
							ref.enqueue_event(events::robot_fault_detected());
							std::cout<<indenting_str_<<name_<<" enqueued fault event"<< std::endl;
						}
					}
				}
				...
				virtual void handle_event(cnc_action_completed const &evnt)
				{
					...
					if(!evnt.success_)
					{
						ref.enqueue_event(events::cnc_fault_detected());
						ref.process_event(events::material_load_failed());
						//ref.process_event(events::robot_fault_detected());
					}
					else
				...
				virtual void handle_event(gripper_action_completed const &evnt)
				{
					...
					if(!evnt.success_)
					{
						ref.enqueue_event(events::gripper_fault_detected());
						ref.process_event(events::material_load_failed());
						//ref.process_event(events::robot_fault_detected());
					}
					else
				...
				// on entry method (specialized to handle material load request events
					void on_entry(events::material_load_requested const &evnt , MaterialLoad_ &sm)
				{
					static BackendType &ref = static_cast<BackendType&>(*this);
					std::cout<< SubmachineInterface::indenting_str_ <<"Entering "<<SubmachineInterface::name_<<" sub-machine\n";
